...
class userController extends Controller
{
    function getProfile() 
    {
        ...
        $result =  $con->getUserProfile($user["mobile"]);
    ...
    function signIn(){
        ...
        $key = "lequoctrang";
        $user = $con->getUserProfile($mobile);
    ...
    function signUp(){
        ...
        if (strlen($data['password']) < 10)
            throw new Exception("Độ dài mật khẩu ít nhấtlà 10!", 400);
        ...
        $password = $data["password"];
        if($con->checkUserExistence($mobile)){
            throw new Exception("Người dùng đã tồn tại", 400);
        }
    ...
    function setAvatar(){
        ...
        $con->updateAvatarName($avatar, $user["mobile"]);
